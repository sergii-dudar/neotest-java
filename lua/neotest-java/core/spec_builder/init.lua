...
local project_name_resolver = require("neotest-java.util.project_name_resolver")
	...
	return {
		...
		build_spec = function(args, config)
			...
			if args.strategy == "dap" then
				...
				-- Resolve project name using multiple strategies:
				-- 1. Parse module's pom.xml or gradle.build
				-- 2. Use directory name as last resort
				local project_name = project_name_resolver.resolve_project_name(module.base_dir, project_type)
